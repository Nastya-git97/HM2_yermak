...
Original file is located at
    https://colab.research.google.com/drive/1ofaMA4FiKYe9h5FVsc6zE5HsHB0IZ8dB
...
def digits_to_words(text):
    ...
    return ' '.join([digit_map[c] for c in text if c in digit_map])
...
def to_mixed_case(name):
    ...
    mixed_words = [words[0].lower()] + [word.capitalize() for word in words[1:] if word]
...
def get_work_time(df):
    df['WorkTime'] = df['SubmitTime'] - df['AcceptTime']
...
def calculated_work_time(df):
    ...
    min_accept_time = grouped['AcceptTime'].min()
    max_submit_time = grouped['SubmitTime'].max()
    return max_submit_time - min_accept_time
...
def extract_revenue(df, column, name):
    df[name] = (df[column].apply(price_dec) / 1000000).round(2)
    return df
